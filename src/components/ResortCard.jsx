...
import { Heart, Mountain, ArrowRight } from "lucide-react";
...
export default function ResortCard({ resort, compact = false }) {
  ...
  if (compact) {
    return (
      <Link
        to={`/resort/${resort.id}`}
        className="group flex-shrink-0 w-64 rounded-xl overflow-hidden bg-peak-card border border-white/5 hover:border-peak-blue/30 transition-all duration-300 hover:-translate-y-1"
      >
        ...
        <div className="p-4">
          ...
          <p className="mt-2 text-peak-red font-semibold text-sm">From €{resort.priceFrom}/day</p>
        </div>
      </Link>
    );
  }
