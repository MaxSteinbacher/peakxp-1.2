...
// When coming from trip context, steps collapse: Location is pre-filled & skipped.
const STEPS_WITH_LOCATION    = ["Location", "Specifications", "Choose Shop", "Checkout"];
const STEPS_WITHOUT_LOCATION = ["Specifications", "Choose Shop", "Checkout"];
...
const FACILITIES = [
  {
    id: "st1", name: "Verbier Main Station — Ski Locker Hall",
  ...
  },
  {
    ...
    image: "https://picsum.photos/seed/storage2/600/400",
  ...
  {
    id: "st3", name: "Medran Gondola Base — Slope-side Lockers",
    image: "https://picsum.photos/seed/storage3/600/400",
  ...
  },
  {
    ...
    image: "https://picsum.photos/seed/storage4/600/400",
...
export default function StorageTab({ agentServiceDetails = {}, onBook }) {
  ...
  // ── Derive trip context ──────────────────────────────────────────────────
  const tripResortName = session?.destination?.label
    || session?.resorts?.[0]?.resortName
    || "";

  // If we have a trip session with a resort, pre-fill location and skip step 0
  const hasTripContext = Boolean(tripResortName);

  // Steps shown depend on whether we have trip context
  const STEPS_KEYS = hasTripContext ? STEPS_WITHOUT_LOCATION : STEPS_WITH_LOCATION;

  // ── State ────────────────────────────────────────────────────────────────
  // step is always 0-based relative to STEPS_KEYS
  const [step, setStep] = useState(0);
  const [location, setLocation] = useState(hasTripContext ? tripResortName : "");
  const [locating, setLocating] = useState(false);
  const [areaTypes, setAreaTypes] = useState([]);
  const [prefs, setPrefs] = useState({
    duration: "Full day", startDate: "", startTime: "08:00",
    endDate: "", endTime: "18:00", items: [], lockers: 1, accessible: false,
  });
  ...
  const toggleAreaType    = (v) => setAreaTypes(prev => prev.includes(v) ? prev.filter(x => x !== v) : [...prev, v]);
  const toggleFilterType  = (v) => setFilterTypes(prev => prev.includes(v) ? prev.filter(x => x !== v) : [...prev, v]);
  ...
  // ── Season locker path ──────────────────────────────────────────────────
  if (prefs.duration === "Season locker" && step === (hasTripContext ? 1 : 2)) {
    ...
    return (
      <BookingShell steps={STEPS_KEYS} current={Math.min(step, STEPS_KEYS.length - 1)} onBack={goBack} onGoToStep={goToStep}>
  ...
  return (
    <BookingShell steps={STEPS_KEYS} current={Math.min(step, STEPS_KEYS.length - 1)} onBack={goBack} onGoToStep={goToStep}>
      ...
      {/* ── STEP 0 — Location (only shown when no trip context) ── */}
      {!hasTripContext && step === 0 && (
        <div className="max-w-2xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
            {[
              { key: "station", label: "At the ski station", desc: "Right next to the lifts — ideal for gear you use daily." },
              { key: "town",    label: "In town / village",  desc: "More options, often lower prices, easy walk." },
            ].map(opt => (
      ...
      {hasTripContext && step === 0 && (
        <div className="flex items-center gap-2 bg-peak-surface border border-white/8 rounded-xl px-4 py-3 mb-6">
          <CheckCircle className="h-4 w-4 text-peak-green flex-shrink-0" />
          <p className="text-sm text-peak-text">
            Finding storage near <strong className="text-peak-text">{location}</strong>
            <span className="text-peak-text-secondary text-xs ml-1">(from your trip)</span>
          </p>
        </div>
      )}

      {/* ── STEP 0 when hasTripContext OR STEP 1 otherwise — Preferences ── */}
      {((hasTripContext && step === 0) || (!hasTripContext && step === 1)) && (
        <div className="max-w-2xl">
          ...
          <button onClick={() => setStep(s => s + 1)} className="mt-8 w-full py-3 bg-peak-red hover:bg-peak-red-hover text-white font-display font-bold text-sm rounded-xl transition-colors">
            Search facilities
          </button>
        </div>
      )}

      {/* ── STEP 1 (hasTripContext) or STEP 2 — Listings ── */}
      {((hasTripContext && step === 1) || (!hasTripContext && step === 2)) && (
        <div>
              ...
              <div>
                ...
                {["Slope-side", "Village centre", "Train station", "Hotel"].map(locType => (
                  <label key={locType} className="flex items-center gap-2 cursor-pointer mb-2">
                    <Checkbox checked={filterTypes.includes(locType)} onCheckedChange={() => toggleFilterType(locType)}
                      className="border-peak-text-secondary data-[state=checked]:bg-peak-blue data-[state=checked]:border-peak-blue" />
          ...
          <div className="mt-8 flex gap-3">
            <button onClick={() => setStep(s => s + 1)} disabled={!selectedFacility}
            ...
            {selectedFacility && (
              <SavePlanButton planData={{
                ...
                estimatedPriceEUR: Math.round(selectedFacility.pricePerDay * rentalDays * prefs.lockers),
              }} />
            )}
          </div>
        </div>
      ...
      {/* ── STEP 2 (hasTripContext) or STEP 3 — Checkout ── */}
      {((hasTripContext && step === 2) || (!hasTripContext && step === 3)) && (
        <CheckoutFlow
