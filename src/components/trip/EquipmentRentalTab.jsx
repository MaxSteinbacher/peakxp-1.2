...
import { Search, SlidersHorizontal, Star, MapPin, ChevronDown, ChevronUp, Check, X, Mountain } from "lucide-react";
...
// ─── Destination search with proximity ──────────────────────────────────────
const ALPINE_REGIONS = [
  { id: "at-tyrol", label: "Tyrol, Austria", sublabel: "Austria", flag: "🇦🇹", lat: 47.18, lng: 11.30 },
  { id: "at-salzburg", label: "Salzburg, Austria", sublabel: "Austria", flag: "🇦🇹", lat: 47.55, lng: 13.20 },
  { id: "at-vorarlberg", label: "Vorarlberg, Austria", sublabel: "Austria", flag: "🇦🇹", lat: 47.35, lng: 9.90 },
  { id: "ch-valais", label: "Valais, Switzerland", sublabel: "Switzerland", flag: "🇨🇭", lat: 46.35, lng: 7.30 },
  { id: "ch-graubuenden", label: "Graubünden, Switzerland", sublabel: "Switzerland", flag: "🇨🇭", lat: 46.70, lng: 9.90 },
  { id: "fr-savoie", label: "Savoie, France", sublabel: "France", flag: "🇫🇷", lat: 45.30, lng: 6.50 },
  { id: "fr-haute-savoie", label: "Haute-Savoie, France", sublabel: "France", flag: "🇫🇷", lat: 46.10, lng: 6.35 },
  { id: "it-alto-adige", label: "South Tyrol, Italy", sublabel: "Italy", flag: "🇮🇹", lat: 46.55, lng: 11.55 },
  { id: "it-trentino", label: "Trentino, Italy", sublabel: "Italy", flag: "🇮🇹", lat: 46.10, lng: 11.00 },
];

function DestinationSearch({ value, onSelect, placeholder = "Search resort or region..." }) {
    ...
    <div ref={ref} className="relative flex-1 min-w-[220px] max-w-sm">
      <div className="relative">
        ...
        <input
          ...
          onFocus={() => { if (results.length) setOpen(true); else if (!query) setOpen(true); }}
        ...
        />
        {query && (
          <button onClick={handleClear} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-peak-text-secondary hover:text-peak-text">
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-peak-surface border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden">
          ...
          {!query && (
            <div className="p-3 border-b border-white/5">
              <p className="text-peak-text-secondary text-xs mb-2 px-1">Alpine regions</p>
              <div className="flex flex-wrap gap-1.5">
                {ALPINE_REGIONS.slice(0, 5).map(r => (
            ...
            <div className="max-h-64 overflow-y-auto">
              {results.map((item, i) => (
                <button key={i} onClick={() => handleSelect(item)}
                  ...
                  <span className="text-base flex-shrink-0">{item.flag || (item.type === "region" ? "🏔️" : "⛷️")}</span>
          ...
          {query && results.length === 0 && (
            <div className="px-4 py-3 text-peak-text-secondary text-sm">No results for "{query}"</div>
          )}
        </div>
      )}
    </div>
...
export default function EquipmentRentalTab({ agentServiceDetails = {}, onBook }) {
  ...
  // Local destination — never read from session (avoids contamination from home search)
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 w-full">
      ...
      <div className="mb-6">
        ...
        <p className="text-peak-text-secondary text-sm">
          {destinationLabel ? `Rental shops near ${destinationLabel}` : "Browse rental shops — select a resort to filter by location"}
          {days > 1 ? ` · ${days} days` : ""}
        </p>
      </div>
