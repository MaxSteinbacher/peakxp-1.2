...
export default function EquipmentRentalTab({ agentServiceDetails = {}, onBook }) {
  ...
  const [wizardSelectedShop, setWizardSelectedShop] = useState(null);
      ...
      {wizardActive && (
        <div className="mt-6">
          <button onClick={() => setWizardActive(false)} className="flex items-center gap-1.5 text-peak-text-secondary hover:text-peak-text text-sm mb-6 transition-colors group">
            ...
            {wizardStep === 2 && (
              <Step2Shops
                selectedShop={wizardSelectedShop}
                setSelectedShop={setWizardSelectedShop}
                answers={wizardAnswers}
                onContinue={() => { setWizardShop(wizardSelectedShop); setWizardStep(3); }}
              />
            )}
            {wizardStep === 3 && (
              <Step3Checkout
