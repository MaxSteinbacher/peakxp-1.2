...
function loadSDK() {
  // Always resolve immediately if SDK already loaded
  ...
  return new Promise((resolve, reject) => {
    ...
    if (existingScript) {
      // Script tag exists but SDK may not be ready yet
      const check = setInterval(() => { if (window.maptilersdk) { clearInterval(check); resolve(window.maptilersdk); } }, 50);
...
export default function ResortRoutePlanner() {
  ...
  useEffect(() => {
    ...
    loadSDK().then(sdk => {
      ...
      // Prevent map from stealing page focus/scroll
      if (mapRef.current) {
      ...
      map.on("load", async () => {
        ...
        // Add terrain DEM source and enable 3D terrain correctly
